...
then
	...
	dd if=/dev/zero of=disk_images/techos.flp bs=512 count=2880 || exit
	vnconfig svnd3 disk_images/techos.flp && newfs_msdos -f 1440 svnd3c && vnconfig -u svnd3 || exit
fi
...
dd conv=notrunc if=source/bootloader/boot.bin of=disk_images/techos.flp || exit


echo "Copying TechOS kernel and programs..."

rm -rf tmp-loop
vnconfig svnd3 disk_images/technos.flp || exit

mkdir tmp-loop && mount -t msdos /dev/svnd3c tmp-loop && cp source/kernel.bin tmp-loop/ && cp source/techosk.sys tmp-loop/
