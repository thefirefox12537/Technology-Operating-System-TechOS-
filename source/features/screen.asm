...
.redbox:				; Loop to draw all lines of box
	...
	mov bh, 0
	mov cx, 55
	mov bl, [FS:CFG_DLG_OUTER_COLOUR]		; White on red
	mov al, ' '
	int 10h
	popa
...
.one_button:
	mov bl, [FS:CFG_DLG_INNER_COLOUR]		; Black on white
	mov dh, 14
	mov dl, 35
	mov si, 8
	mov di, 15
...
.two_button_wait:
	...
	mov bl, [FS:CFG_DLG_OUTER_COLOUR]		; White on red for cancel button
	mov dh, 14
	mov dl, 42
	mov si, 9
	mov di, 15
...
.noleft:
	...
	mov bl, [FS:CFG_DLG_OUTER_COLOUR]		; Black on white
	mov dh, 14
	mov dl, 27
	mov si, 8
	mov di, 15
	...
	mov bl, [FS:CFG_DLG_INNER_COLOUR]		; White on red for cancel button
	mov dh, 14
	mov dl, 43
	mov si, 8
	mov di, 15
...
.noright:
	...
	.cancel_button_string	db ' Batal', 0
	.ok_button_noselect	db '   OK   ', 0
	.cancel_button_noselect	db '    Batal   ', 0
